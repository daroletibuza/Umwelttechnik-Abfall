...
Der TC-Gehalt liegt laut den Messergebnissen bei $\approx 25\%$ bzw. aus der Summe zwischen TIC und TOC bei  $\approx 23\%$ (siehe Tabelle \ref{tab:tc}). Aufgrund der optischen Zusammensetzung des Mülls teilweise enthaltenen Papier, Pappe und Kunststoffschnipseln sowie den Textilfasern und ascheähnlichen Resten könnte der Kohlenstoffgehalt plausibel sein. Verglichen mit Tabelle \ref{tab:tc_vergleich} \cite[S.11]{HansGunterRamke.} scheint sich die Plausibilität mit den Daten für Feinmüll gemischt mit Pappe, Papier und Kunststoffen zu decken.
%Tabelle START
\vspace*{-3.5mm}
\renewcommand{\arraystretch}{1.2}
\begin{table}[h!]
	\centering
	\caption{Elementgehalte von Restabfall und ausgewählter Fraktionen \cite[S.11]{HansGunterRamke.}}
...
\begin{figure}[h!]
	...
	\caption{Darstellung eines Hausmülls durch unterschiedliche Stoffgruppen und deren Aufteilung auf die Komponenten Wasser, Inertstoff, Kunststoff und sonstigen organischen Komponenten aus \cite[S.23]{scholz2013}}
...
\begin{table}[h!]
	\centering
	\caption{Tabellenausschnitt mit Heizwerten üblicher Brennstoffe aus \cite{S.Furkus.}}
	\label{tab:heizwerte}
	%\resizebox{10cm}{!}{
...
\textcolor{red}{VORHER BRENNWERT SCHREIBEN }\\


Der Heizwert der Probe siedelt sich mit $\approx \SI{5}{\mega \joule \per \kg}$ vergleichsweise zwischen die Müllfraktionen von Feinmüll mit $\approx \SI{4,29}{\mega \joule \per \kg}$ und Holz, Vegetablien mit $\approx \SI{5,83}{\mega \joule \per \kg}$ an \cite{scholz2013}. Aus rein optischer Einschätzung scheint lediglich der Feinmüll als plausibel, da Holz bzw. Vegetabilien nicht vorrangig zu erkennen sind. Die untersuchte Müllprobe liegt somit unter dem Heizwert für allgemeinen Hausmüll mit $\approx \SI{8,00}{\mega \joule \per \kg}$ \cite{scholz2013}. Das wiederum erscheint durch den hohen Inertstoffgehalt als sinnvoll, nimmt man den Messwert der Müllfraktion von Papier, Pappe, etc. $\approx \SI{11,96}{\mega \joule \per \kg}$ in die Betrachtung des Heizwertes hinzu \cite{scholz2013}. Aufgrund der unterschiedlichen Zusammensetzungen von Müllproben erscheint eine genaue Plausibilitätsbetrachtung als schwierig. Die Dimensionen dieser Müllprobe, was den Heizwert angelangt scheinen jedoch nicht unrealistisch zu sein.\\
\textcolor{red}{Vergleich mit üblichen Brennstoffen weiter schreiben}\\
Ausgehend von der Sinnhaftigkeit der erhobenen Daten lässt der Heizwert, ähnlich wie der Brennwert, ebenfalls mit üblichen Werten von Brennstoffen der Tabelle \ref{tab:heizwerte} vergleichen.

 \cite{S.Furkus.}.\\

\textcolor{red}{Muss mal noch schön mit eingebunden werden}\\

Die Zusammensetzung der beiden Proben ist nicht genauer bestimmt. Es werden lediglich Aussagen zur qualitativen Beschaffenheit getroffen. Ein Widerspruch ist hier nicht zu erkennen, da die Werte immerhin in der selben Größenordnung liegen.
...
\begin{itemize}	
	...
	\textcolor{red}{Fehlerbetrachtung: Heizwert nur grobe Einschätzung durch Näherungsformel}
...
Chlorgehalte des Hausmülls schwankten mitte der 1990ger Jahre zwischen \SI{0,4}{\mpercent} und \SI{1}{\mpercent} \cite{scholz2013}.\\
